...
## Companion page: vesi/ (geneesi.com/vesi/)

A separate small page, `vesi/index.html`, GENERATED by `python vesi/gen_vesi.py` - do not hand-edit the page,
edit the generator. It lifts the imp, orb and bond shader blocks verbatim out of index.html (so a shader
change there needs a regenerate) and wraps them in its own JS: drag H / O from the list onto a plane
perpendicular to the camera (z = 0), billiard bounce, a RULES table (H+H → H2, O+O → O2, H+O → OH, OH+H → H2O,
H2+O → H2O, H2+OH → H2O+H, H2+O2 → H2O+O, O2+H → OH+O, OH+OH → H2O+O), products of one reaction get a 0.4 s
grace period against EACH OTHER only, water counter top-right. Drawing: Geneesi's shell (IMPRAD, glass
everywhere), nucleon cluster (NUC_BIAS 2), rigidOrbSet lobes with the SEEDATTR per-molecule seed, rebuilt into
four thin-instance meshes every frame. Each molecule spins about z and rocks +-30 deg about an in-plane axis.
`window.gVesi` exposes spawn/mols/scene for tests. `vesi/chk_vesi.js` is the node --check dump (gitignored).
